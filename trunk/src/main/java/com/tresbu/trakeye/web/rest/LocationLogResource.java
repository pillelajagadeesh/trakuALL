...
//		    	locationlog.setUserId(getCurrentUserLogin());
    ...
    public ResponseEntity<List<LiveLogs>> getlatestLocationLogs()
        throws URISyntaxException, ParseException {
    	    	log.debug("REST request to get a page of LocationLogs");
    	List<LiveLogs> liveLogs = locationLogService.listLiveLogs(getCurrentUser().getLogin(),Instant.now().minusMillis(TimeUnit.MINUTES.toMillis(5)).toEpochMilli(),ZonedDateTime.now(ZoneOffset.UTC).getHour());
